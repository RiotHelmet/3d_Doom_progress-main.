...
class Player {
  ...
  constructor(_id, _userID, _username) {
...
function raycast(origin, dir) {
  ...
  for (let i = 0; i < hitObjects.length; i++) {
    if (hitObjects[i].distance < closestDist) {
      closestObject = hitObjects[i];
      closestDist = closestObject.distance;
    }
  }
...
function CirclevsOBB(_Circle, _OBB) {
  ...
  if (
    ...
    _Circle.position.z + 90 <= _OBB.position.z + _OBB.height
  ) {
  ...
  for (current = 0; current < vertices.length; current++) {
    ...
    if (
      ((vc.y > _Circle.position.y && vn.y < _Circle.position.y) ||
        (vc.y < _Circle.position.y && vn.y > _Circle.position.y)) &&
      _Circle.position.x <
        ((vn.x - vc.x) * (_Circle.position.y - vc.y)) / (vn.y - vc.y) + vc.x
    ) {
      collision = !collision;
    }
  }
  ...
  if (collision) {
    if (
      _Circle.position.z + 90 > _OBB.position.z + _OBB.height &&
      _OBB.position.z + _OBB.height > _Circle.position.z
    ) {
...
function updatePlayerMovement() {
  ...
  if (keys["ArrowLeft"]) {
    player.rotation -= deltaT * sensitivity;
